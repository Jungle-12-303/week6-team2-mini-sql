...
// FILE *out : test 했을때 터미널에 출력 뿐만 아니라 테스트할때 출력 파일에 실재 출력된게 맞는지 파일에 저장하고 검사하기 위해? 
/*
출력 결과를 캡처해서 테스트 코드가 비교할 수 있게 하는 그릇입니다.
짧게 말하면:
터미널 출력은 사람이 봄
임시 파일 출력은 테스트 코드가 검사함
그래서 FILE *out이 있으면 실제 실행과 테스트를 같은 함수로 처리할 수 있습니다.
*/
